...
var registerCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if tmp_u == "" || tmp_p == "" || tmp_m == "" || tmp_c == "" {
			fmt.Println("Please input your username[-u=xxx], password[-p=xxx], email[-m=xxx], cellphone[-c=xxx]")
			return
		}
		pass, err := service.UserRegister(tmp_u, tmp_p, tmp_m, tmp_c)
		if pass == false {
			fmt.Println("The username you input has existed!")
			return
		} else {
			if err != nil {
				fmt.Println("Some unexpected errors happened when try to record your info,Please check errors.")
				return
			} else {
